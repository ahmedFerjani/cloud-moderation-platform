...
def test_extract_image_id() -> None:
    assert dlq_services.extract_image_id({}) == "unknown"
    assert dlq_services.extract_image_id({"s3_key": "uploads/abc-123.jpg"}) == "abc-123"


# Verifies failure records omit image_hash when the value is missing or invalid.
def test_store_failure_omits_invalid_image_hash() -> None:
...
def test_store_failure_omits_whitespace_image_hash() -> None:
    with patch.object(dlq_services.table, "put_item") as mock_put:
...
def test_store_failure_includes_valid_image_hash() -> None:
    with patch.object(dlq_services.table, "put_item") as mock_put:
    ...
    assert item["image_hash"] == "abc123"
    assert item["failure_reason"] == "failed"


# Verifies failure reason defaults to moved_to_dlq when no explicit error is provided.
def test_store_failure_defaults_reason_when_missing() -> None:
    with patch.object(dlq_services.table, "put_item") as mock_put:
        dlq_services.store_failure({"s3_key": "uploads/img-1.jpg"})
        item = mock_put.call_args.kwargs["Item"]

    assert item["failure_reason"] == "moved_to_dlq"
